...
class Order extends Element
{
    ...
    public function afterSave(bool $isNew)
    {
        ...
        $orderRecord->phone = $this->getPhone() ?: '';
    ...
    public function getPhone()
    {
        if($this->_phone){
            ...
            if ($this->getCustomer() && $this->customerId) {
                $phoneinfo = Craft::$app->db->createCommand('SELECT phone FROM craft_commerce_orders WHERE customerId='.$this->customerId)
               ->queryOne();
                if(!empty($phoneinfo) && isset($phoneinfo['phone']) && $phoneinfo['phone']){
                    $this->setPhone($phoneinfo['phone']);
                }else{
