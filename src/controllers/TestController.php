    ...
    public function actionUpdateCart()
    {
        ...
        if (!$this->_cart->getUser() && $phone = $this->request->getParam('phone')) {
            ...
            $userinfo = Craft::$app->db->createCommand('SELECT * FROM craft_users WHERE phone='.$phone)
               ->queryOne();
            if(!empty($userinfo)){
                // return $this->setFailFlash('用户已存在');
                return Craft::$app->session->setFlash('email','用户已存在');
            }
            // 判断验证码是否正确
            // $error = '验证码输入有误';
            ...
            $codeinfo = Craft::$app->db->createCommand('SELECT * FROM craft_chinacms WHERE phone='.$phone)
               ->queryOne();
            if(empty($codeinfo) || $codeinfo['code']!=$code || (time()-$codeinfo['update_time'])>600){
                // return $this->setFailFlash('验证码输入有误');
                return Craft::$app->session->setFlash('email','验证码输入有误');
            }
