...
class CartController extends BaseFrontEndController
{
    ...
    public function actionUpdateCart()
    {
        ...
        // Set guest email address onto guest customers order.
        if (!$this->_cart->getUser() && $phone = $this->request->getParam('phone')) {

            $codetrue = $this->request->getParam('codetrue');
            $code = $this->request->getParam('code');

            Craft::$app->session->setFlash('joinphone',$phone);
            Craft::$app->session->setFlash('joincode',$code);

            if($codetrue==='0'){
                return Craft::$app->session->setFlash('email',Craft::t('app', 'Please enter correct verification code.'));
            }

            Craft::$app->session->setFlash('email',Craft::t('app', 'The mobile phone number has been registered. Please log in directly.'));

            $userinfo = Craft::$app->db->createCommand('SELECT * FROM craft_users WHERE phone='.$phone)
               ->queryOne();
            if(!empty($userinfo)){
                return Craft::$app->session->setFlash('email',Craft::t('app', 'The mobile phone number has been registered. Please log in directly.'));
            }
            $codeinfo = Craft::$app->db->createCommand('SELECT * FROM craft_chinacms WHERE phone='.$phone)
               ->queryOne();
            if(empty($codeinfo) || $codeinfo['code']!=$code){
                return Craft::$app->session->setFlash('email',Craft::t('app', 'Please enter correct SMS code.'));
            }
            if(isset($codeinfo['update_time']) && (time()-$codeinfo['update_time'])>300){
                return Craft::$app->session->setFlash('email',Craft::t('app', 'SMS code is expired, please request again.'));
            }
            $this->_cart->setPhone($phone);
        }
